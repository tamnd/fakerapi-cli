// Package cli assembles the fakerapi command tree from the fakerapi
...
// NewApp assembles the kit application from the fakerapi domain. The
...
//
// To add a command, declare it in fakerapi/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := fakerapi.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(fakerapi.Domain{}).Register(app)
