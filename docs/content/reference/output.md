...
describes what users get. Pick a format with `-o`, or let fakerapi choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
fakerapi <command> -o table   # aligned columns for reading
fakerapi <command> -o jsonl   # one JSON object per line, for piping
fakerapi <command> -o json    # a single JSON array
fakerapi <command> -o csv     # spreadsheet friendly
fakerapi <command> -o tsv     # tab-separated
fakerapi <command> -o url     # just the URL column
fakerapi <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
fakerapi <command> --fields id,title,url
```
...
```bash
fakerapi <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
fakerapi <command>            # a table, because this is a terminal
fakerapi <command> | wc -l    # JSONL, because this is a pipe
```
